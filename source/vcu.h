...
typedef struct {
    ...
    volatile uint32_t MC_POST_FAULT;    // CAN 1
    volatile uint32_t MC_RUN_FAULT;     // CAN 1
    volatile int16_t MC_CURRENT;        // CAN 1
    volatile int16_t MC_VOLTAGE;        // CAN 1
    volatile int16_t MC_SPEED;          // CAN 1
    ...
    volatile uint8_t CHARGER_CONNECTED; // CAN 0

    // TODO - read input from CAN bus
    volatile int16_t BMS_VOLTAGE;       // CAN 0

    // TODO - read input from CAN bus
    volatile int16_t BMS_TEMPERATURE;   // CAN 0
...
typedef struct {
    ...
    // TODO - put signal on CAN bus
    volatile uint8_t PRECHARGE_FAILED;  // CAN 0
